...
홈페이지(countnine/musebase-home)의 "Sync latest release version" 워크플로우를
`repository_dispatch`로 즉시 깨워 "Latest release: vX.Y.Z" 문구를 갱신한다.
(추가 안전망: 홈페이지는 매일 06:00 UTC 크론으로도 최신화된다.)

> **전제:** 이 리포에 시크릿 `HOMEPAGE_DISPATCH_TOKEN`(musebase-home 쓰기 권한 PAT)이
...
- [ ] 홈페이지 버전 자동 갱신 확인(`notify-homepage.yml` 실행 → musebase-home Actions). 실패 시 `.\scripts\notify-homepage.ps1`로 대체
